...
struct greater_cust {
    bool operator()(const State_best& a, const State_best& b){
...
class Tsp_bxb_best {
    ...
    public:
    Tsp_bxb_best(Graph& g, bool DEBUG, int MAX_TIME);
    int solve();
...
Tsp_bxb_best::Tsp_bxb_best(Graph& g, bool DEBUG, int MAX_TIME){
    this->_g = &g;
    debug = DEBUG;
...
inline vector<int> Tsp_bxb_best::nearestNeighbourTSP()
{
    vector<int> path;
...
inline void Tsp_bxb_best::bfs_cycle(int si)
{
    State_best root = {{si}, 0, 0, si};
    priority_queue<State_best, vector<State_best>, greater_cust> q;
    // queue<State> q;
    q.push(root);
...
inline int Tsp_bxb_best::getBestRes()
{
    return bestRes;
}

inline int Tsp_bxb_best::solve() 
{   
