...
class Tsp_bxb_dfs {
    ...
    public:
    Tsp_bxb_dfs(Graph& g, bool debug, int MAX_TIME);
    int solve();
    void hamil_cycle_step(int vi);
    int getBestRes();
};

Tsp_bxb_dfs::Tsp_bxb_dfs(Graph& g, bool DEBUG, int MAX_TIME) {
    this->_g = &g;
    debug = DEBUG;
...
inline void Tsp_bxb_dfs::hamil_cycle_step(int vi)
{   
    ...
    for(int ni : v_neighs) {
        if(visited[ni] == 0 && ni != si) {
            ...
            //jeśli koszt dojscia do sąsiada większy niż dotychczasowy najlepszy koszt cyklu to pomin
            if(nextNode.weight >= bestRes) continue;
...
inline int Tsp_bxb_dfs::getBestRes()
{
    return bestRes;
}
